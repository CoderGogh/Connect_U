...
public class FollowController {
    ...
    @Operation(summary = "자신을 팔로우하는 유저 목록 조회", description = "페이지 번호(startOffset), 페이지 크기(pageSize)를 URL 쿼리 파라미터로 전달해주세요.")
    public ResponseEntity<PagingResultDto<UsersListResponseDto>> getMyFollowers(@CurrentUsersId Integer usersId, @RequestParam(value = "startOffset", defaultValue = "0") Integer startOffset, @RequestParam(value = "pageSize", defaultValue = "1") Integer pageSize) {
        return ResponseEntity.ok(followService.followerList(usersId, startOffset, pageSize));
    }

    @GetMapping("/my-followings")
    @Operation(summary = "자신이 팔로우하는 유저 목록 조회", description = "페이지 번호(startOffset), 페이지 크기(pageSize)를 URL 쿼리 파라미터로 전달해주세요.")
