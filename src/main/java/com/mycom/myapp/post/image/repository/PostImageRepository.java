...
public interface PostImageRepository extends JpaRepository<PostImage, PostImageKey> {
	...
	List<PostImage> findByPostAndIsDeletedFalseAndImageKeyIsNotNullOrderByIdSeq(PostEntity postEntity);
	...
	List<String> findImageKeysByPost(@Param("postEntity") PostEntity postEntity);

	// 현재 post에 대해 최대 seq 값을 조회 (없으면 -1 반환)
	@Query("SELECT COALESCE(MAX(p.id.seq), -1) FROM PostImage p WHERE p.post = :postEntity")
	Integer findMaxSeqByPost(@Param("postEntity") PostEntity postEntity);
